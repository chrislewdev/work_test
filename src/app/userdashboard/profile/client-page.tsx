...
export default function DashboardClientPage() {
  ...
  const { loading: profileLoading, error: profileError } = profileState;
  ...
  const handlePersonalInfoUpdate = async (data: any) => {
    try {
      // Reset profile state before updating to ensure clean slate - consistent pattern
      resetState.profile();

      // Wait for the update to complete
      const result = await updateProfile(data);

      // Only close the editing mode if update was successful
      if (result) {
        // Give a short delay so user can see success message
        setTimeout(() => {
          setEditingPersonalInfo(false);
        }, 1500);
      }
      // If result is null (failure), keep the form open so user can see the error
    } catch (error) {
      console.error("Error updating profile:", error);
      // Keep form open so user can see error
    }
  };
